...
public class MediaPlayerCtrl : MonoBehaviour
{
	...
	public void Resize()
	{
        ...
        float width = Call_GetVideoWidth();
        float height = Call_GetVideoHeight();

        Vector2 size = this.GetComponent<RectTransform>().sizeDelta;

        float final_width = size.x;
        float final_height = size.y;

        float ratio = width / Convert.ToSingle(height);

        if (ratio > (size.x / size.y)){
            final_width = size.x;
            final_height = size.x / ratio;
        }else if (ratio < (size.x / size.y)) {
            final_height = size.y;
            final_width = ratio * final_height;
        }

        this.GetComponent<RectTransform>().sizeDelta = new Vector2(final_width,final_height);
